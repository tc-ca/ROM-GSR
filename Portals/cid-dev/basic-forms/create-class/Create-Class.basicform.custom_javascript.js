...
$(document).ready(function () {
    ...
    for (var i in list) {
        ...
        if (item.substr(0, 6) == "refid=") {
            siteid = item.substr(6);
            break;
        }
    }

    //$("#ovs_site_name").attr("value", );

    $("#ovs_site").attr("value", siteid);

    $("#ovs_site_entityname").attr("value", "account");
    // insert button
    ...
    // hide controls 
    tdg.c.control_hide("ovs_primaryclass", true);
    tdg.c.control_hide("ovs_site", true);
    tdg.c.control_hide("cid_modifiedbyregistrant", true);
    tdg.c.control_hide("ovs_compatibility_group", true);
    tdg.c.control_hide("ovs_erapcategory");
...
function btn_save_new_onclick() {
    ...
    var ovs_primaryclass = $("#ovs_primaryclass").val();
    if (ovs_primaryclass == null) return;

    // insert
    var contact_id = '{{user.id}}';
    var ovs_compatibility_group = $("#ovs_compatibility_group").val();

    ovs_siteClass_insert(siteId, ovs_primaryclass, ovs_compatibility_group, contact_id);
    $("#btn_save_new").prop('disabled', false);
}
...
function ovs_primaryclass_selected(text, id) {
    ...
    $("#ovs_primaryclass").attr("value", id);
    $("#ovs_primaryclass_name").attr("value", text);
    $("#ovs_primaryclass_entityname").attr("value", 'ovs_primaryclass');
...
function ovs_siteClass_insert(siteId, ovs_primaryclass, ovs_compatibility_group, contact_id) {
    debugger;

    if (ovs_compatibility_group == "") {
        var data = {
            "ovs_Site@odata.bind": "/accounts(" + siteId + ")",
    ...
    tdg.webapi.create("ovs_siteclasses", data, success_cb, error_cb);
}
...
function form_clear() {
    ...
    try {
        $("#ovs_primaryclass").attr("value", null);
        $("#ovs_primaryclass_name").attr("value", null);
