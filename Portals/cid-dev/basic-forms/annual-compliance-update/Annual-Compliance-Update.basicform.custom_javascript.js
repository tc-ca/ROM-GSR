	...
	entityFormClientValidate = function () {
		...
		if (anniversaryDate == null) {
			...
			if (anniversaryDate > today)
				anniversaryDate = new Date(anniversaryDate.getFullYear() - 1, anniversaryDate.getMonth(), anniversaryDate.getDate());

			var dateDiff = Math.floor((Date.UTC(today.getFullYear(), today.getMonth(), today.getDate())
				- Date.UTC(anniversaryDate.getFullYear(), anniversaryDate.getMonth(), anniversaryDate.getDate())) / (1000 * 60 * 60 * 24))

			if (dateDiff < 30) { //Annual Compliance Update can only be completed on or 30 days after the Company’s Anniversary Date each year
				complienceReadonly = true;
			}
		}
