...
var TemplateMainForm = (function (window, document) {
 ...
 function PublishTemplate(executionContext) {
    ...
    //First check if the template is already published
     ...
     var message = Xrm.Utility.getResourceString(resexResourceName, "qm_sytemplate.Publishing.ConfirmationMessage");
     var title = Xrm.Utility.getResourceString(resexResourceName, "qm_sytemplate.Publishing.ConfirmationTitle");
     var successMessage = Xrm.Utility.getResourceString(resexResourceName, "qm_sytemplate.Publishing.SuccessMessage");
     var failMessage = Xrm.Utility.getResourceString(resexResourceName, "qm_sytemplate.Publishing.FailMessage");
         ...
         .then(
             function success(result) {
                 if (result["statuscode"] != 930840000)
                 {
                         ...
                         function (success) {
                             if (success.confirmed) {
                                 ...
                                 Xrm.WebApi.online.updateRecord("qm_sytemplate", recordId, template).then(
                                     function success(result)
                                     {
                                         glHelper.DisplayFormNotification(successMessage, "INFO");
                                     },
                                     function (error) {
                     ...
                     glHelper.DisplayFormNotification(failMessage, "ERROR", 5000);
...
function CloneTemplate(executionContext)
{
    ...
    var message = Xrm.Utility.getResourceString(resexResourceName, "qm_sytemplate.Cloning.ConfirmationMessage");
    var title = Xrm.Utility.getResourceString(resexResourceName, "qm_sytemplate.Cloning.ConfirmationTitle");
