...
var WOC_RJ = (function (window, document) {
    ...
    function verifySelectedWOC(selectedRefs, selectedCount, LCID, gridControl, formConrol) {
    ...
    function displayForHQ(selectedRefs, selectedCount, LCID, gridControl, formConrol) {
        ...
        //verify if selected has HQ flag true 
        // and local is not true already
        if (row1.hqflag != true) {
            Xrm.Navigation.openErrorDialog({ message: "Selected candidates is not HQ-approwed.\r Please re-select" });
            return;
        }

        //check local is not set on true for HQ approved
        if (row1.hqflag == true && row1.regionflag == true) {
            Xrm.Navigation.openErrorDialog({ message: "The HQ-approved candidate '" + row1.Name + "' have been sabstituted already.\r Please select a different candidate." });
            return;
        }

        //call replacement manager, pass row1 object
        var manager = formContext.getControl('WebResource_CandidateReplacement');
        if (manager) {
            manager.getContentWindow().then(
                function (contentWindow) {
                    contentWindow.setHQ_Candidate(formContext, gridControl, row1);
                }
            )
        }
    }

    function displayForLocal(selectedRefs, selectedCount, LCID, gridControl, formConrol) {
        ...
        // and local is not true already
        if (row1.hqflag == true) {
            Xrm.Navigation.openErrorDialog({ message: "Selected candidates is HQ-approwed.\r Please select a Local Candidate" });
            return;
        }

        //check local is not set on true for HQ approved
        if (row1.regionflag == true ) {
            Xrm.Navigation.openErrorDialog({ message: "This Local Candidate candidate '" + row1.Name + "' have been a part of another sabstitution.\r Please select a different candidate." });
            return;
        }
        ...
        if (manager) {
            manager.getContentWindow().then(
                function (contentWindow) {
                    contentWindow.setLocalCandidate(formContext, gridControl, row1);
                }
            )
        }
    }

    function isHQ(selectedRefs, gridControl) {
        ...
        //verify if selected has HQ flag true 
        // and local is not true already
        if (row1.hqflag != true) return false;        

        //check local is not set on true for HQ approved
        if (row1.hqflag == true && row1.regionflag == true) return false;
        
        return true;
    }

    function isLocal(selectedRefs, gridControl) {
        ...
        //check local is not set on true for HQ approved
        if (row1.regionflag == true) return false;
    ...
    return {
        ...
        displayForHQ: displayForHQ,
        displayForLocal: displayForLocal,
        isHQ: isHQ,
        isLocal: isLocal,
    };
