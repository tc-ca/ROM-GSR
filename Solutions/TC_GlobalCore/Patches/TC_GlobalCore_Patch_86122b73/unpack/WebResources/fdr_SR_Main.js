...
var SR_main = (function (window, document) {
    ...
    return {
        ...
        OnSite_Change: function (executionContext) {
            ...
            if (hSite != null) {
                var customerFilter = "<filter type='and'><condition attribute='ovs_siteid' operator='eq' value='" + hSite[0].id.replace('{', '').replace('}', '') + "'/></filter>";
                customerFilter = "<filter type='and'><condition attribute='ovs_siteid' operator='in'><value>2FC50EC5-98A5-4A4C-B83B-24A3A590FAFC</value></condition></filter>";
                formContext.getControl("fdr_operation").addCustomFilter(customerFilter, "ovs_mocregistration");
            }
            ...
            if (hOperation != null) {
                ...
                Xrm.WebApi.online.retrieveRecord("ovs_mocregistration", opperationid, "?$select=fdr_operationstatus,_ovs_siteid_value").then(
                    function success(result) {
                        ...
                        //set site lookup
                        if (hSite == null) {
                            glHelper.SetHeaderLookup(formContext, "fdr_site", logicalname, siteid, name);
                        }
                    },
                    function (error) {
