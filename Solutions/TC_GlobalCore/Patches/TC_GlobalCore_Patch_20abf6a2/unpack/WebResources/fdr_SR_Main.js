...
var SR_main = (function (window, document) {
    ...
    var statusMappingActive = {
        ...
        "Post Inspection Review": ["Post Inspection Review", "Additional Info Required", "Refused", "Approved"],
        "794600004": [794600004, 794600000, 794600008, 794600007],
    ...
    var statusMappingFullRevocation = {
        ...
        "Approved": ["Approved", "Technical Review", "Closed","Canceled"],
        "794600007": [794600007, 794600002, 2, 794600001]
    };

    var statusMappingCloseActiveRegistration = {

        "Admin Review": ["Admin Review", "Closed","Canceled"],
        "794600006": [794600006, 2, 794600001]
    };
    ...
    function SetStateChangebility(formContext) {
        ...
        //if status reason was Draft, Cancellation Pending, or Completed or Approved then user can change State
        if (SR_status == 1
            || SR_status == 794600009 || SR_status == 794600005 || SR_status == 794600007)
    ...
    return {
                ...
                if (serviceRquestType != null) {
                    ...
                    if (serviceRquestType == 794600003 || serviceRquestType == 794600004 || serviceRquestType == 794600005 || serviceRquestType == 794600006) 
                    {
                        ...
                        if (serviceRquestType == 794600004)
                        {
                            ...
                            var currentStatus = glHelper.GetOptionsetValue(formContext, "statuscode");
                        ...
                        //If close expired or close active then Admin Review is available
                        if (serviceRquestType == 794600005 || (serviceRquestType == 794600006))
                        {
                            //If Type is Full Revocation, only "Approved" should be available status reason.
                            var currentStatus = glHelper.GetOptionsetValue(formContext, "statuscode");

                            glHelper.filterOptionSetUsingOrigin(formContext, "header_statuscode", SR_Status_origin, statusMappingCloseActiveRegistration["794600006"], true);
                        }
                    }
        ...
        OnStatusReason_Change: function (executionContext) {
            ...
            if (currentState != 1) {
                glHelper.filterOptionSetUsingOrigin(formContext, "header_statuscode", SR_Status_origin, statusMappingActive[currentStatus], true)
        ...
        OnState_Change: function (executionContext) {
            ...
            else
            {
                ...
                //If Type is Close Active Registration
                else if (serviceRquestType == 794600005)
                {
                    glHelper.filterOptionSetUsingOrigin(formContext, "header_statuscode", SR_Status_origin, statusMappingCloseActiveRegistration["794600006"], true);
                }
                else
        ...
        OnOperation_Change: function (executionContext) {
            ...
            if (hOperation != null) {
                ...
                ServiceRquestTypeFilter(formContext, null);
                glHelper.SetValue(formContext, "fdr_containertype", null);
